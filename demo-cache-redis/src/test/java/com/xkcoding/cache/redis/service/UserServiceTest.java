...
public class UserServiceTest extends SpringBootDemoCacheRedisApplicationTests {
     ...
     */
    @Test
    public void getTwiceByRedisTemple() {
        // 模拟查询id为1的用户
        User user1 = userService.getByRedisTemple(1L);
        log.debug("【user1】= {}", user1);

        // 再次查询
        User user2 = userService.getByRedisTemple(1L);
        log.debug("【user2】= {}", user2);
        // 查看日志，只打印一次日志，证明缓存生效
    }
     ...
     */
    @Test
    public void getAfterSaveByRedisTemple() {
        userService.saveOrUpdateByRedisTemple(new User(4L, "测试中文"));

        User user = userService.getByRedisTemple(4L);
        log.debug("【user】= {}", user);
        // 查看日志，只打印保存用户的日志，查询是未触发查询日志，因此缓存生效
    }
     ...
     */
    @Test
    public void deleteUserByRedisTemple() {
        // 查询一次，使redis中存在缓存数据
        userService.getByRedisTemple(1L);
        // 删除，查看redis是否存在缓存数据
