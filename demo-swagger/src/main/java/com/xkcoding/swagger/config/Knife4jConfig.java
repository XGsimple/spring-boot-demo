...
/**
 * Knife4j配置
 * https://blog.csdn.net/justry_deng/article/details/129461986
 *
...
public class Knife4jConfig implements WebMvcConfigurer {
    ...
    public Docket docket() throws UnknownHostException {
        ...
        Docket docket = new Docket(DocumentationType.SWAGGER_2).apiInfo(new ApiInfoBuilder()
                                                                            ...
                                                                            // 服务地址
                                                                            .termsOfServiceUrl(
                                                                                "http://local.idea-aedi.com/")
                                                                            // 作者及联系信息
                                                                            .contact(new Contact("JustryDeng",
                                                                                                 "https://gitee.com/JustryDeng",
                                                                                                 "13548417409@163.com"))
                                                                            // api版本
                                                                            .version("1.0.0").build())
                                                               //分组名称(微服务项目可以用微服务名分组)
                                                               ...
                                                               .apis(RequestHandlerSelectors.basePackage(
                                                                                                getProjectBasePackage())
                                                                                            .and(RequestHandlerSelectors.withClassAnnotation(
                                                                                                                            RestController.class)
                                                                                                                        .or(RequestHandlerSelectors.withClassAnnotation(
                                                                                                                            Controller.class))))
    ...
    private List<SecurityContext> securityContexts() {
        ...
        result.add(SecurityContext.builder().securityReferences(securityReferences).forPaths(
            // 当直接使用swagger文档发送请求时，这些api需要满足securityReferences认证要求
            PathSelectors.regex("/.*").and(
                // 当直接使用swagger文档发送请求时，这些api不需要满足securityReferences认证要求. '.*'表示匹配所有
                PathSelectors.regex("/hello.*").or(PathSelectors.regex("/hi.*")).negate())).build());
        return result;
    }
    ...
    public static class Knife4jSwaggerEnumPlugin implements ModelPropertyBuilderPlugin, ParameterBuilderPlugin {
        ...
        static {
            ...
            Objects.requireNonNull(modelPropertyBuilderDescriptionField,
                                   "ModelPropertyBuilder_descriptionField should noe be null.");
        ...
        public void apply(ModelPropertyContext context) {
            ...
            List<String> enumDescList = Arrays.stream(enumConstants)
                                              .map(this::obtainEnumDescription)
