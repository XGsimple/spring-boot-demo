...
public class ChainDropDownWriteHandler implements SheetWriteHandler {
    ...
    private final Map<Integer, ChainDropDownResolved> map;

    public ChainDropDownWriteHandler(Map<Integer, ChainDropDownResolved> map) {
        this.map = map;
    }
    ...
    public void afterSheetCreate(WriteWorkbookHolder writeWorkbookHolder, WriteSheetHolder writeSheetHolder) {
        ...
        for (Map.Entry<Integer, ChainDropDownResolved> e : map.entrySet()) {
            // k 为存在下拉数据集的单元格下表 v为下拉数据集
            Integer k = e.getKey();
            ChainDropDownResolved v = e.getValue();
            CellRangeAddressList rangeList = new CellRangeAddressList(v.getFirstRow(), v.getLastRow(), k, k);
            Sheet hideSheet = getSheet(workbook, v.getTypeName());
            if (v.isRootFlag()) {
                Row firstRow = hideSheet.createRow(v.getRowIndex());
                List<String> values = v.getDataMap().get(ChainDropDownResolved.ROOT_KEY);
