package com.xkcoding.excel.exceltool.dropdown.service;
...
public interface IChainDropDownService {
    default ChainDropDownResolved getSource(boolean isRoot, String... params) {
        Map<String, List<String>> dataMap = new HashMap<>();
        if (isRoot) {
            // 第一层，没有父级
            List<String> root = getRoot(params);
            dataMap.put(ChainDropDownResolved.ROOT_KEY, root);
        } else {
            // 有父级，key=父级，value=子集
            Map<String, List<String>> parentBindSubMap = getParentBindSubMap(params);
            dataMap.putAll(parentBindSubMap);
        }
        return new ChainDropDownResolved().setRootFlag(isRoot).setDataMap(dataMap);
    }
